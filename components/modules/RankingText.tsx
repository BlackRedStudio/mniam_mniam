...
function RankingText({ranking}: TRankingTextProps) {
    ...
    const totalProductsTextVariant = handleMultiplePersonText(ranking.totalProductsRateCount);
    const mniamPointsTextVariant = handleMultiplePersonText(ranking.mniamPoints);

    const productsText = ['produkt', 'produkty', 'produktów'][
        totalProductsTextVariant
    ];

    const pointsText = ['punkt', 'punkty', 'punktów'][
        mniamPointsTextVariant
    ];

    return (
        <>
            <div>Oceniłeś do tej pory <strong className='text-orange'>{ranking.totalProductsRateCount}</strong> {productsText},</div>
            ...
            <div>do <strong className='text-orange'>{ranking.imgUploadedCount}</strong> przesłałeś <strong>zdjęcie</strong>.</div>
            <div className="mt-3 text-xl">Łącznie zdobyłeś <strong className='text-orange'>{ranking.mniamPoints}</strong> {pointsText}.</div>
